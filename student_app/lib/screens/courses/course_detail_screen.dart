...
class _CourseDetailScreenState extends State<CourseDetailScreen> {
  ...
  Widget _buildContent(BuildContext context, CartProvider cartProvider) {
    ...
    final isInCart = cartProvider.cart?.items.any((item) => item.courseId == course.id) ?? false;

    // Check enrollment — use both myCourses (fully enrolled) and myEnrollments (all stages)
    final isFullyEnrolled = cartProvider.isEnrolled(course.id) ||
    ...
    final enrollment = cartProvider.myEnrollments.any((e) => e.courseId == course.id)
        ? cartProvider.myEnrollments.firstWhere((e) => e.courseId == course.id)
        : null;
    final hasEnrollment = enrollment != null;

    final myCourseInstance = cartProvider.myCourses.any((c) => c.id == course.id)
        ? cartProvider.myCourses.firstWhere((c) => c.id == course.id)
        : null;
    final isBlocked = myCourseInstance?.isBlocked ?? false;
    final blockReason = myCourseInstance?.blockReason ?? 'Access suspended by Administrator';
    final remainingAmount = myCourseInstance?.remainingAmount ?? 0.0;
    final paidAmount = myCourseInstance?.paidAmount ?? 0.0;
        ...
        Expanded(
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                CachedNetworkImage(
                  imageUrl: course.thumbnail ?? 'https://via.placeholder.com/600x300',
                  height: 240,
                    ...
                    children: [
                      if (isBlocked) ...[
                        Container(
                          ...
                          child: Row(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Icon(Icons.gpp_bad_rounded, color: Colors.red[700], size: 28),
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          Text(
                                            ...
                                            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.red[900]),
                                          ),
                                        ],
                                      ),
                                    ],
                        ...
                        Container(
                          ...
                          child: Row(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Icon(Icons.pending_actions_rounded, color: Colors.orange[700], size: 28),
                              const SizedBox(width: 12),
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    ...
                                    if (enrollment != null && enrollment.payment != null) ...[
                                      ...
                                      ] else if (enrollment.payment!.status == 'submitted') ...[
                                        const SizedBox(height: 12),
                                        Row(
                                          children: [
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 6),
                          Text('Instructor: ${course.instructorName}', style: const TextStyle(color: Colors.grey)),
                        ],
                      ),
                      ...
                      ...course.sections.map((section) {
                        ...
                        final subLecturesCount = subSectionsList.fold<int>(0, (sum, s) => sum + s.modules.length);
                        final totalLecturesCount = directLectures.length + subLecturesCount;

                        final directDuration = directLectures.fold<int>(0, (sum, m) => sum + (m.duration ?? 0));
                        final subDuration = subSectionsList.fold<int>(0, (sum, s) => sum + s.modules.fold<int>(0, (sSum, m) => sSum + (m.duration ?? 0)));
                          ...
                          child: ExpansionTile(
                            ...
                            children: [
                              ...
                              if (directLectures.isNotEmpty)
                                ...directLectures.map((module) => _buildModuleTile(context, module, isFullyEnrolled, isBlocked, blockReason)),
                              ...
                              if (subSectionsList.isNotEmpty) ...[
                                ...
                                ...subSectionsList.map((sub) {
                                  final subDur = sub.modules.fold<int>(0, (sum, m) => sum + (m.duration ?? 0));
                                  return Card(
                                    ...
                                    child: ExpansionTile(
                                      ...
                                      children: [
                                        ...
                                        if (sub.modules.isEmpty)
                                          ...
                                          ...sub.modules.map((module) => _buildModuleTile(context, module, isFullyEnrolled, isBlocked, blockReason)),
                                      ],
                                    ),
                                  );
                                }),
                      ...
                      if (course.modules.where((m) => m.sectionId == null).isNotEmpty) ...[
                        const SizedBox(height: 12),
                        Card(
                          margin: const EdgeInsets.only(bottom: 12.0),
                          elevation: 0,
                          ...
                          child: ExpansionTile(
                            ...
                            children: [
                              ...course.modules
                                  ...
                                  .map((module) => _buildModuleTile(context, module, isFullyEnrolled, isBlocked, blockReason)),
                            ],
                          ),
                        ),
                      ],
          ...
          child: Row(
            children: [
              if (!isFullyEnrolled) ...[
                Expanded(
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Tuition Fee', style: TextStyle(color: Colors.grey, fontSize: 12)),
              ...
              Expanded(
                child: SizedBox(
                  height: 52,
                  child: _buildActionButton(context, cartProvider, isInCart, isFullyEnrolled, hasEnrollment),
                ),
              ),
            ],
          ),
  ...
  Widget _buildModuleTile(BuildContext context, CourseModuleModel module, bool isFullyEnrolled, bool isBlocked, String blockReason) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Stack(
          ...
          children: [
            Icon(getModuleIcon(), color: canAccess ? getModuleColor() : Colors.grey[400], size: 24),
            if (module.isFree && !isBlocked)
              Positioned(
                ...
                child: Container(
                  ...
                  child: const Icon(Icons.lock_open, color: Colors.white, size: 8),
                ),
              ),
          ],
        ),
        ...
        trailing: canAccess
            ? IconButton(
                icon: Icon(
                  ...
                  color: module.isFree ? Colors.green : AppConstants.primaryColor,
                  size: 28,
                ),
        ...
        onTap: isBlocked
            ...
            : (canAccess
                ...
                : () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('👑 Premium content — enroll to unlock this lesson'),
  ...
  Future<void> _launchModuleContent(BuildContext context, CourseModuleModel module) async {
    if (module.type == 'video') {
      if (module.youtubeUrl == null || module.youtubeUrl!.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No YouTube link available for this video.')),
        );
        return;
      }
    ...
    // Handle document / PDF materials with external browser
    ...
    try {
      if (await canLaunchUrl(uri)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Could not open the lecture content: $urlToLaunch')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildActionButton(
    ...
    if (hasEnrollment) {
      return ElevatedButton.icon(
        ...
        label: const Text('Track Admission', style: TextStyle(fontWeight: FontWeight.bold)),
        style: ElevatedButton.styleFrom(
              ...
              try {
                ...
                if (mounted) {
                  ...
                  if (msg.contains('ALREADY_PURCHASED')) {
                    ...
                    displayMsg = 'Enrollment approval is pending for this course.';
                  } else if (msg.contains('ALREADY_IN_CART')) {
                    displayMsg = 'This course is already in your cart.';
                  } else if (msg.contains('ALREADY_ENROLLED')) {
                    displayMsg = 'You already have an enrollment request for this course.';
                  } else {
                    displayMsg = msg;
                  }
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(displayMsg), backgroundColor: Colors.red[700]),
                  );
                }
              }
            },
